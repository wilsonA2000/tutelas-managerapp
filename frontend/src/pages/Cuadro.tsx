...
// Búsqueda insensible a acentos y mayúsculas (espejo del `unaccent()` del backend):
// "garcia" matchea "GARCÍA", "tramite" matchea "EN TRÁMITE", etc.
...
const ALL_COLUMNS = [
  ...
  { key: 'PARTE_RESOLUTIVA_1ST', label: 'Resolutiva 1ra', width: 280 },
  ...
  { key: 'PARTE_RESOLUTIVA_2ND', label: 'Resolutiva 2da', width: 280 },
...
export default function Cuadro() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      const s = norm(search)
      rows = rows.filter(r => Object.values(r).some(v => norm(v).includes(s)))
    }
    for (const [col, val] of Object.entries(colFilters)) {
      if (val) {
        const v = norm(val)
        rows = rows.filter(r => norm(r[col] || '').includes(v))
      }
    }
    return rows
            ...
            {sorted.map((row, idx) => (
              <tr key={row.id as number} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50`}>
                ...
                {COLUMNS.map(col => {
                  ...
                  // ACCIONANTE: decora con icono pequeño si hay acumulación.
                  const isAccionante = col.key === 'ACCIONANTE'
                  const tipoAcum = isAccionante ? (row.tipo_acumulacion as string | null) : null
                  const acumA = isAccionante ? (row.acumulado_a_case_id as number | null) : null
                  ...
                  return (
                    <td key={col.key}
                      ...
                      title={
                        tipoAcum === 'RECTOR' ? `Rector de acumulación. ${val}` :
                        tipoAcum === 'ACUMULADO' && acumA ? `Acumulado al expediente #${acumA}. ${val}` :
                        (val || 'Vacío — click para editar')
                      }>
                      {tipoAcum === 'RECTOR' && (
                        <Link2 size={9} className="inline-block text-indigo-600 mr-0.5 align-baseline" />
                      )}
                      {tipoAcum === 'ACUMULADO' && (
                        <Link2 size={9} className="inline-block text-amber-600 mr-0.5 align-baseline" />
                      )}
