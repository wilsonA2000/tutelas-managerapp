...
@router.get("/tokens")
def api_token_stats(db: Session = Depends(get_db)):
    """Estadísticas de consumo de tokens con análisis de ahorro."""
    from backend.agent.token_manager import get_token_stats, get_savings_report
    stats = get_token_stats(db)
    savings = get_savings_report(db)
    return {
        "stats": {
...
def api_set_budget(body: BudgetRequest, db: Session = Depends(get_db)):
    """Configurar presupuesto de tokens."""
    from backend.agent.token_manager import TokenBudget, check_budget
    budget = TokenBudget(daily_limit_usd=body.daily_limit_usd, monthly_limit_usd=body.monthly_limit_usd)
    allowed, reason = check_budget(db, budget)
    return {"budget": {"daily": body.daily_limit_usd, "monthly": body.monthly_limit_usd},
            "status": "OK" if allowed else "EXCEEDED", "message": reason}
