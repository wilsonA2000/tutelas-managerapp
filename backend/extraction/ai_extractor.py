...
def extract_with_ai(documents: list[dict], folder_name: str = "",
    ...
    for doc in sorted_docs:
        ...
        if not text:
            continue
        # Caps subidos para DeepSeek (ventana 64K tokens ≈ ~200K chars, 2026-06-22); antes eran
        # cotas del 4B en CPU. Solo se trunca docs MUY largos; head+tail conserva inicio
        # (partes/hechos) y final (RESUELVE). En multi-doc el presupuesto total lo controla
        # el caller; aquí solo se evita que UN doc gigante (cientos de pp.) coma toda la ventana.
        if not _is_critical_pdf(doc.get("filename", "")) and len(text) > 170000:
            text = text[:130000] + "\n[...CONTENIDO TRUNCADO...]\n" + text[-30000:]
