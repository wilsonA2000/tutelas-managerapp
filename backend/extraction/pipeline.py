"""Pipeline de extraccion v8 (LEGACY) — carpeta → texto → IA → DB.

(Modernización Fase 7) Sin uso desde v9: la extracción de campos la hace
`backend/v9/pipeline.py`. `process_folder` + sus auxiliares quedan aquí solo porque
`unified*.py` (también legacy) los usan; candidato a borrado en la limpieza final.
Las utilidades de documentos que SÍ usa el código vivo (`extract_document_text`,
`classify_doc_type`, `verify_document_belongs`, etc.) se movieron a `doc_ops.py`;
aquí se re-importan para no romper a los consumidores legacy.
"""
...
# Fase 7.5a — los cuerpos de estas funciones viven ahora en doc_ops.py; se re-exportan
# aquí para no romper a unified.py / unified_cognitive.py / ir_builder.py / tests.
from backend.extraction.doc_ops import (  # noqa: F401
    extract_document_text, classify_doc_type, reextract_document,
    compute_file_hash, detect_duplicate_documents,
    verify_document_belongs, verify_all_documents, _verify_bayesian, _verify_legacy,
)
