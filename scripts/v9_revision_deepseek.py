#!/usr/bin/env python3
"""Revisión + diligenciamiento del cuadro de tutelas con DeepSeek-V3 (API DeepSeek).

Por cada expediente, envía al modelo TODO el texto de sus documentos (priorizado y
...
Requiere DEEPSEEK_API_KEY en el entorno:
    DEEPSEEK_API_KEY='sk-...' ./venv/bin/python3 scripts/v9_revision_deepseek.py --limit 3
    DEEPSEEK_API_KEY='sk-...' ./venv/bin/python3 scripts/v9_revision_deepseek.py --apply --workers 8
"""
...
def _ask_deepseek(client, model: str, system: str, user: str) -> dict | None:
    import time as _t
    ...
    for attempt in range(_RETRY_MAX):
        try:
            ...
            break
        except (_oa.RateLimitError, _oa.APIStatusError, _oa.APIConnectionError, _oa.APITimeoutError) as e:
            status = getattr(e, "status_code", None)
            if attempt == _RETRY_MAX - 1:
                return {"__error__": f"{type(e).__name__}: {str(e)[:160]}"}
            wait = 30 if status == 429 else 5 * (attempt + 1)
...
def main() -> int:
    ...
    ap.add_argument("--workers", type=int, default=8,
                    help="hilos concurrentes (DeepSeek tolera bastante concurrencia)")
    ap.add_argument("--model", default=_MODEL_DEFAULT)
    args = ap.parse_args()

    api_key = os.getenv("DEEPSEEK_API_KEY", "").strip()
    if not api_key:
        print("ERROR: define DEEPSEEK_API_KEY en el entorno.", file=sys.stderr)
        return 2
    ...
    try:
        ...
        def _work(job):
            res = _ask_deepseek(client, args.model, _SYSTEM, job["prompt"])
            return job, res
        ...
        rpt = reports_dir / f"revision_deepseek_{ts}.md"
